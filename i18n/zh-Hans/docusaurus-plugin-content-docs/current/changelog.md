...
- **移除 TUI 组件**：原有终端 UI 交互正式下线，替换为更轻量的 CLI 命令行交互模式（#2710）
- **跨平台串口工具支持**：原生集成跨平台串口交互能力，支持嵌入式硬件调试场景（#2673）
- **MCP 能力全面增强**：新增完整的 MCP CLI 命令体系，完善 HTTP 会话管理和本地服务支持
- **交互体验统一优化**：全渠道工具反馈动效统一，新增思考内容可见性切换、配置保存重启提示等功能
- **自定义模型端点支持**：CLI 新增自定义 OpenAI 兼容端点配置能力

### 功能

#### 核心架构与 Agent
- 新增结构化提示词分层能力，支持按能力槽管理工具提示词（#2656）
- 新增聊天消息 tool_calls 字段原生支持
- 统一推理内容和工具调用可见性控制，支持全局或单次会话开关
- 新增 MCP slash 命令和工具详情查询能力

#### 工具能力
- 新增跨平台串口硬件工具（#2673）
- MCP 工具新增完整 CLI 管理命令：show/add/list/remove/test/edit
- Web 端新增文件下载功能
- 工具反馈新增独立消息模式，支持 JSON 格式参数代码块展示
...
#### CI/CD 与构建
- CI 流程拆分标签创建和发布为独立工作流
- 新增 stale bot 自动关闭无活跃度的 Issue 和 PR
- macOS CGO 构建改为并行执行
- Docker 镜像标签统一改为小写

### Bug 修复

- 修复 MCP 工具调用时发送 null 而非空对象的兼容性问题
- 修复 MCP HTTP 会话重试逻辑和客户端生命周期管理
- 修复 MCP 本地 stdio 服务器家目录路径展开问题
- 修复 MCP 初始化失败无错误提示问题
- 修复多轮 Windows 构建失败问题
- 修复 Docker 构建流程适配 Go 版本变更问题
- 修复 Windows 启动器子进程控制台闪烁问题
- 修复 Darwin 串口工具交叉编译问题
- 优化 Unix 平台串口取消和超时轮询逻辑
- 优化 Windows 平台串口 I/O 处理逻辑
- 修复 Telegram OAuth 链接 HTML 转义问题
- 修复飞书图片下载 API fallback 逻辑，新增图片上传支持
- 修复 cron 任务 sessionKey 传播问题
- 修复飞书消息附件展示异常
- 修复 DeepSeek v4 思考历史记录丢失问题
- 修复工具调用在思考 UI 中展示错位问题
- 修复 Seahorse SQLite reasoning_content 持久化失败问题
- 修复启动器本地化修改污染共享 web-search 路由问题
- 多轮代码去重，减少冗余函数
- 修复 govet shadow 声明错误
- 优化配置错误提示与 web 搜索 fallback 逻辑
- 修复 PID=1 时的单例检查问题

### 构建与运维

- AWS SDK 升级（bedrockruntime、config）
- 飞书 SDK 升级
- zerolog 1.35.0 → 1.35.1
- 前端依赖升级：i18next、react-i18next、typescript-eslint、vite
